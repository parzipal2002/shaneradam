  ...
  return (
    <header
      className={`fixed inset-x-0 top-0 z-50 border-b hairline transition-colors ${
        scrolled
          ? "bg-paper-surface/90 dark:bg-ink-surface/90 backdrop-blur-md"
          : "bg-transparent"
      }`}
    >
      <div className="mx-auto flex max-w-6xl items-center gap-1 px-4 sm:px-6">
        ...
        <nav
          className="hidden flex-1 items-stretch overflow-x-auto md:flex"
          aria-label="Primary"
        >
          {nav.map((n) => {
                ...
                <Circle
                  className={`h-1.5 w-1.5 ${
                    isActive
                      ...
                      : "fill-transparent text-ink/20 dark:text-paper/20"
                  }`}
                />
      ...
      {mobileOpen && (
        <nav
          className="border-t hairline bg-paper-surface dark:bg-ink-surface md:hidden"
          aria-label="Mobile"
        >
          {nav.map((n) => (
            <button
